...
class GovernmentSocialMediaAnalyzer(object):
    ...
    def create_tw_party_table(self):
        ...
        self.create_party_followers_count_bar_chart(df)
    ...
    def create_party_friends_count_bar_chart(self, df):
        ...
        py.plot(data, filename=self.__country_code+'-tw-party_friends_count_bar')

    def create_party_followers_count_bar_chart(self, df):
        data = [
            go.Bar(
                x=df.Party, # assign x as the dataframe column 'x'
                y=df.FollowersCount
        ...
        py.plot(data, filename=self.__country_code+'-tw-party_followers_count_bar')
    ...
    def create_party_politicans_count_pie_chart(self, df):
        ...
        data = [trace]
        py.plot(data, filename=self.__country_code+'-tw-party_politicans_count_pie')
...
def table_chooser(analyzer):
    table_id = input("Choose 1 for Twitter table, 2 for gov-api table or 3 for both")
    if table_id == "1":
        analyzer.create_tw_politican_table()
        analyzer.create_tw_party_table()
    elif table_id == "2":
        analyzer.create_govapi_politican_table()
    else:
        analyzer.create_govapi_politican_table()
        analyzer.create_tw_politican_table()
        analyzer.create_tw_party_table()
...
def main():
    ...
    while True:
        nat_id = input("Choose 1 for CH and 2 for UK and 3 for exit")

        if nat_id == "1":
            analyzer_ch = GovernmentSocialMediaAnalyzer("CH")
            table_chooser(analyzer_ch)
        elif nat_id == "2":
            analyzer_uk = GovernmentSocialMediaAnalyzer("UK")
            table_chooser(analyzer_uk)
        elif nat_id == "3":
            break
        else:
